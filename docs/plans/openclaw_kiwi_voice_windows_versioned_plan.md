...
Telegram approval adapter fixture v7.6:
  ...
  - Telegram callback data는 `approve|reject:<requestId>:<payloadHashTail>` 형식으로 제한
...
execution: v7.6.1 approve와 v7.6.2 reject/duplicate live button smoke 완료, runner는 dry-run만 수행
```

완료 기준:

- pending request를 Telegram message + inline keyboard JSON으로 렌더링한다.
- synthetic callback으로 approved/rejected queue 전이가 가능하다.
...
- live Reject button은 `approvalMethod=telegram`, `rejectedBy=telegram:8194519852`, `rejectedAt`을 기록한다.
- already rejected request의 duplicate callback은 `ignored`로 처리되고 approved duplicate를 만들지 않는다.
