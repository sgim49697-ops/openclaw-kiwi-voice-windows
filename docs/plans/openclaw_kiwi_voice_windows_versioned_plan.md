...
- Gateway URL을 `ws://localhost:18789`로 설정
- Node Mode와 Start with Windows 설정
- WSL에서 pending device 승인
- `allowCommands`는 최소 명령만 허용
...
`system.run`은 v3에서 wrapper 정책까지 준비한 뒤 허용한다.
...
- 안전한 테스트 페이지에서 click/type/fill/select 검증
