...
```text
현재 canonical 기준: v7.8 Voice → Telegram Approved Browser Read Live E2E
...
Live smoke: voice-origin dispatcher notify 1회, voice-origin browser_read docs.openclaw.ai 1회, approved runner browser_read example.com 1회 통과
...
다음 gate: Browser read allowlist 템플릿화 또는 voice → approved Codex plan gate
```
...
task debug:forever        # STOP 파일 또는 Ctrl+C 전까지 supervised cycle 반복
task debug:stop           # .debugloop/STOP 생성
task debug:forever-status # latest forever summary 출력
task debug:forever-clear-stop # STOP 파일 제거
task approval:queue       # 승인 대기 요청 목록 출력
task approval:status      # 승인 결과 확인
task e2e:dry-run          # 실제 실행 없이 routing preview
task e2e:approved-run     # manual approved request 1건 실행
task e2e:approved-run-all # manual approved request 전체 실행
task voice:dry-run        # 실행 없이 wake/STT/intent/approval request까지만
task browser:probe        # isolated openclaw profile read probe
task browser:interact-probe # isolated openclaw profile safe fixture interact probe
task browser:cdp-recovery # page-level CDP timeout 시 isolated profile 자동 복구
```

`task --watch`는 빠른 read-only check에만 사용한다. long-running child process나 실제 실행 task에는 사용하지 않는다.

`debug:autoloop`는 repo 파일을 직접 수정하지 않는다. 현재 tracked/untracked 파일을 스캔해 정책/스키마/Python compile/dry-run/monitor/probe를 자동 구성하고, 결과를 `.debugloop/runs/autoloop.jsonl`과 `.debugloop/runs/latest-summary.md`에 기록한다. 새 스크립트는 기본적으로 compile만 수행하며, 실행은 allowlisted check 또는 `# debug-autoloop: command=...` marker가 있는 경우에만 자동 편입한다. 예외적으로 repo-local voice/Kiwi dry-run 계약은 안전한 no-write smoke로 간주해 `voice_e2e_probe.py`, `kiwi_transcript_dry_run.py`, `kiwi_live_dry_run_probe.py`가 존재하면 자동 실행한다.

browser page-level CDP가 timeout이면 `browser_cdp_recovery.py`가 isolated `openclaw` profile에 한해서 `stop → start → open https://example.com → cdp_probe` 복구를 시도한다. `reset-profile`, cookie/storage 변경, logged-in profile 사용은 금지한다. 3회 연속 복구 실패 후에는 자동 재시작을 중단하고 external blocker로 남긴다.
...
- Browser: isolated openclaw profile read/interact 검증 + CDP recovery 자동화 진행
```
...
openclaw recovery target → page-level CDP timeout 시 stop/start/open/reprobe 자동 시도
openclaw interact → local fixture click/type/fill/select smoke만 허용
```
...
```text
task browser:probe
task browser:interact-probe
task browser:cdp-recovery
python3 scripts/wsl/browser_probe.py
python3 scripts/wsl/browser_interact_probe.py --profile openclaw
python3 scripts/wsl/browser_cdp_recovery.py --profile openclaw --url https://example.com
...
Interact probe는 repo-local fixture를 `127.0.0.1:<free-port>`에 띄우고 `openclaw` profile에서 `click`, `type`, `fill`, `select` 후 fresh snapshot으로 상태 변화를 확인한다.
같은 `openclaw` profile의 browser probes는 병렬 실행하지 않는다. OpenClaw browser refs는 최신 snapshot에 묶이므로 read/interact 검증은 순차 실행한다.
...
기본 Browser lane은 isolated openclaw profile을 사용하고, 개인 user profile은 사용하지 않는다.
page-level CDP timeout은 reset-profile 없이 openclaw stop/start/open/reprobe로만 자동 복구한다.
...
5. openclaw CDP recovery로 browse-read 재검증
6. openclaw fixture로 browse-interact 검증
7. browser-high-impact: upload/download/submit/send/payment은 별도 승인 또는 deny
```

브라우저 루프는 isolated `openclaw` profile을 기본으로 사용한다. `user` profile은 사용자가 현장에 있고 명시적으로 요청한 경우에만 approval queue에 올린다.
...
v7.2 live transcript safety gate:

```text
- 실제 microphone 전에 Kiwi `OPENCLAW_BIN`을 dry-run shim으로 교체한다.
- shim은 `openclaw agent --message <transcript>` 형태만 허용한다.
- transcript는 repo-local `kiwi_transcript_dry_run.py`로만 전달한다.
- 결과는 `.debugloop/runs/kiwi-live-dry-run.jsonl`에 ignored runtime log로 남긴다.
- dispatcher, real OpenClaw agent, browser, node, approval 변경은 실행하지 않는다.
```
...
v7.2 gate는 `debug_autoloop.py`에 `kiwi:live-dry-run`으로 자동 편입한다. 이 probe는 Windows dry-run shim만 호출하고 dispatcher, browser, node, approval 상태를 바꾸지 않는다.
...
approved run은 manual file approval만 사용한다. agent는 pending request를 approved로 바꾸지 않는다. 사용자가 `.debugloop/queue/pending/<id>.json`을 `.debugloop/queue/approved/<id>.json`으로 옮기고 `status`를 `approved`로 바꾼 뒤 `e2e_approved_runner.py` 또는 `task e2e:approved-run REQUEST_ID=<id>`를 호출한다.

approved runner 처리:

```text
dispatcher lane: notify/open_url_readonly/open_vscode_codex_plan/open_app_allowlisted/run_task_recipe
browser lane: browser_read/browser_interact
critical: approved 파일이 있어도 실행 거부
```

browser approved run은 isolated `openclaw` profile만 허용한다. `browser_read`는 safe URL open/snapshot/screenshot까지만 수행하고, `browser_interact`는 local fixture smoke만 수행한다. submit/send/delete/upload/download/login/credential action은 계속 금지한다.

approved runner는 성공한 request마다 `.debugloop/runs/e2e-approved-executed/<requestId>.json`을 남긴다. 같은 request는 기본적으로 재실행하지 않으며, 명시적인 `--force`가 있을 때만 재실행한다. `debug:forever`는 approved request를 자동 실행하지 않고 summary에 queue count만 표시한다.
...
```text
실제 Kiwi Web Microphone 발화가 browser_read approval을 만들고,
Telegram 승인 후 windows-cdp profile에서 docs.openclaw.ai를 read-only로 여는지 검증한다.
```

구현:

```text
kiwi_live_approval_bridge.py가 browser_read 승격 조건을 지원한다.
조건은 wouldExecute=false, action=browser_read, riskTier=low,
params.profile=windows-cdp, params.url=https://docs.openclaw.ai,
payloadHash valid, source=voice-planner-dry-run이다.
```

결과:

- 실제 발화 `"브라우저에서 docs.openclaw.ai 읽어줘"`가 STT를 거쳐 Codex planner `browser_read` preview로 도달했다.
- `kiwi-live-20260604-021431-819` pending request가 생성됐다.
- Telegram live Approve callback이 `approvalMethod=telegram`, `approvedBy=telegram:8194519852`로 approved queue 전이를 만들었다.
- `e2e_approved_runner.py --dry-run`은 browser read probe preview만 출력했다.
- `e2e_approved_runner.py --execute-live --confirm-request-id kiwi-live-20260604-021431-819`로 `windows-cdp`에서 `https://docs.openclaw.ai` open/snapshot/screenshot이 통과했다.
- screenshot artifact는 `.debugloop/artifacts/browser/browser-read-ok.png`로 생성됐다.
- executed marker 생성 후 두 번째 live 실행은 skip 처리됐다.
- wrong expected URL, `https://gmail.com`, `browser_interact` live 시도는 거부됐다.
- cancel/critical transcript는 approval request를 만들지 않았다.
- Web Microphone은 종료 후 `web_audio_clients=0`으로 복귀했다.
- Gateway approvals는 `allowlist + ask=always + askFallback=deny + autoAllowSkills=off` 상태를 유지했다.
