...
- Browser: openclaw profile status/open/tabs OK, raw CDP page commands timeout
```
...
CDP probe는 `/json/version`, `/json/list`, `Browser.getVersion`, `Runtime.evaluate`, `Page.captureScreenshot`, `Accessibility.getFullAXTree`를 직접 호출하고 `.debugloop/artifacts/browser/cdp-probe.json`에 결과를 남긴다.

현재 판정:

```text
CDP browser endpoint는 정상이다.
page target WebSocket은 연결되지만 Runtime/Page/Accessibility command 응답이 timeout된다.
따라서 OpenClaw wrapper timeout만의 문제가 아니라 Chromium/headless/profile 쪽 page-level CDP 처리 문제로 본다.
```
...
5. browse-read: open/snapshot/screenshot
6. browse-interact: click/type/fill/select
7. browser-high-impact: upload/download/submit/send/payment은 별도 승인 또는 deny
```

브라우저 루프는 `openclaw` profile만 기본으로 사용한다. `user` profile은 사용자가 현장에 있고 명시적으로 요청한 경우에만 approval queue에 올린다.
